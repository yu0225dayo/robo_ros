"""
IMU 自動高さ推定 + 把持姿勢生成まで実行するパイプライン

test_demo_w_IMU.py の完全版。SAM-3D メッシュ生成 → 高さ推定 → SAM-6D 姿勢推定 →
Shape2Gesture 把持姿勢生成 まで一括実行する。--skip-grasp は存在せず常に把持姿勢まで行う。

使用方法:
    # フルモード（data-dir 指定）
    python test_demo_w_IMU2.py --mode full \
        --data-dir saved_data/test_20240101_120000
    ...
    # 既存メッシュで pose + grasp のみ（mesh パス指定）
    python test_demo_w_IMU2.py --mode online \
        --data-dir saved_data/test_20240101_120000 \
        --mesh meshes/test_object.ply \
        --server-mesh-path "/path/on/server/test_object.ply" \
        --template-dir "/path/on/server/templates"
"""
...
def main():
    parser = argparse.ArgumentParser(description="IMU 自動高さ推定 + 把持姿勢生成パイプライン")
